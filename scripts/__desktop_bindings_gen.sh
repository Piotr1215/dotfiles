...
#   scripts/__gnome_bindings.conf     GNOME custom keybindings, monitor moves
#
...
WM_KEYS=(
    ...
    "org.gnome.shell.extensions.pop-shell pop-monitor-up|Move window to upper monitor (pop-shell)"
    "org.gnome.shell.extensions.pop-shell pop-monitor-down|Move window to lower monitor (pop-shell)"
    "org.gnome.desktop.wm.keybindings move-to-monitor-left|Move window to left monitor (GNOME)"
    "org.gnome.desktop.wm.keybindings move-to-monitor-right|Move window to right monitor (GNOME)"
    "org.gnome.desktop.wm.keybindings move-to-monitor-up|Move window to upper monitor (GNOME)"
...
gnome_wm_records() {
    ...
    for entry in "${WM_KEYS[@]}"; do
        ...
        # unbound. A missing schema (pop-shell not installed) prints nothing.
        while read -r accel; do
            [ -n "$accel" ] || continue
            # Keypad twins repeat the arrow row and only lengthen the list.
            [[ "$accel" == *KP_* ]] && continue
            printf '%s|%s|gsettings %s %s\n' "$accel" "$label" "$schema" "$key"
        done < <(gsettings get "$schema" "$key" 2>/dev/null | sed 's/^@as //' | tr -d "[]'" | tr ',' '\n')
    done
}
...
if command -v gsettings >/dev/null 2>&1; then
    {
        ...
        { gnome_records; gnome_wm_records; } | python3 -c "$NORMALIZE" gnome
    } > "$GNOME_OUT"
else
