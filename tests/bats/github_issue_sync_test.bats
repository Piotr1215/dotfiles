...
# ====================================================
# TEAM TRIAGE CLEANUP TESTS
# ====================================================

# Helper that installs a mock `task` returning a single +triage task with the
# given linear_issue_id (or empty for a manually tagged task).
_install_triage_task_mock() {
    local linear_id="$1"
    local task_json
    if [[ -z "$linear_id" ]]; then
        task_json='[{"uuid":"task-uuid-1","description":"Manually tagged","status":"pending","tags":["triage"]}]'
    else
        task_json='[{"uuid":"task-uuid-1","description":"Triage feed task","status":"pending","tags":["triage"],"linear_issue_id":"'"$linear_id"'"}]'
    fi
    cat > "${TEST_DIR}/task" <<EOF
#!/bin/bash
if [[ "\$1" == "+triage" && "\$2" == "status:pending" && "\$3" == "export" ]]; then
    echo '$task_json'
elif [[ "\$1" == "rc.confirmation=no" ]]; then
    echo "MOCK: task \$*" >> "${TEST_DIR}/task_commands.log"
fi
...
@test "clean_removed_team_triage_tasks deletes task absent from both feeds" {
    _install_triage_task_mock "DEVOPS-999"

    run clean_removed_team_triage_tasks $'DEVOPS-100\nAI-50' $'DEVOPS-200\nDOC-10'
    [ "$status" -eq 0 ]
    [[ "$output" =~ "no longer in any triage feed" ]]
    grep -q "task-uuid-1 delete" "${TEST_DIR}/task_commands.log"
}

@test "clean_removed_team_triage_tasks keeps task present in main feed (cross-team safety)" {
    # ENGUI Triage assigned to me arrives via main feed with +triage applied.
    # The team-triage feed only carries Dev Ops/AI, so without the dual check
    # this task would be deleted on the same run it was created.
    _install_triage_task_mock "ENGUI-500"

    run clean_removed_team_triage_tasks $'DEVOPS-100\nAI-50' $'ENGUI-500\nDOC-10'
    [ "$status" -eq 0 ]
    # No delete must have been issued
    if [ -f "${TEST_DIR}/task_commands.log" ]; then
        ! grep -q "delete" "${TEST_DIR}/task_commands.log"
    fi
}

@test "clean_removed_team_triage_tasks keeps task present in triage feed" {
    _install_triage_task_mock "DEVOPS-880"

    run clean_removed_team_triage_tasks $'DEVOPS-880\nAI-50' "DOC-10"
    [ "$status" -eq 0 ]
    if [ -f "${TEST_DIR}/task_commands.log" ]; then
        ! grep -q "delete" "${TEST_DIR}/task_commands.log"
    fi
...
@test "clean_removed_team_triage_tasks skips task without linear_issue_id" {
    # A user-managed +triage task with no Linear linkage must not be deleted.
    _install_triage_task_mock ""

    run clean_removed_team_triage_tasks "" ""
    [ "$status" -eq 0 ]
    if [ -f "${TEST_DIR}/task_commands.log" ]; then
        ! grep -q "delete" "${TEST_DIR}/task_commands.log"
    fi
...
@test "clean_removed_team_triage_tasks handles empty +triage task list" {
    cat > "${TEST_DIR}/task" << EOF
#!/bin/bash
if [[ "\$1" == "+triage" && "\$2" == "status:pending" && "\$3" == "export" ]]; then
    echo '[]'
...
EOF
    ...
    run clean_removed_team_triage_tasks "DEVOPS-1" "DEVOPS-2"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "No +triage tasks in taskwarrior" ]]
}

# ====================================================
# TEAM TRIAGE FEED FETCH TESTS
# ====================================================

@test "get_team_triage_issues parses Triage feed JSON correctly" {
    cat > "${TEST_DIR}/curl" << 'EOF'
#!/bin/bash
if [[ "$*" =~ "linear.app" ]]; then
    cat <<'JSON'
{"data":{"issues":{"nodes":[
  {"id":"abc","title":"Triage me","url":"https://linear.app/loft/issue/DEVOPS-880/triage-me","state":{"name":"Triage"},"project":null,"dueDate":null,"priority":4,"cycle":null}
]}}}
JSON
    echo "200"
else
...
EOF
    ...
    run get_team_triage_issues
    [ "$status" -eq 0 ]
    [[ "$output" =~ "DEVOPS-880" ]]
    [[ "$output" =~ "Triage me" ]]
    [[ "$output" =~ "\"status\":\"Triage\"" ]]
}

@test "get_team_triage_issues handles HTTP error codes" {
    cat > "${TEST_DIR}/curl" << 'EOF'
#!/bin/bash
if [[ "$*" =~ "linear.app" ]]; then
...
EOF
    ...
    run get_team_triage_issues
    [ "$status" -eq 1 ]
    [[ "$output" =~ "Linear triage API returned HTTP 500" ]]
}

@test "get_team_triage_issues query targets Dev Ops and AI teams in Triage state" {
    # Capture the request body and assert it carries the correct team filter.
    cat > "${TEST_DIR}/curl" << EOF
#!/bin/bash
if [[ "\$*" =~ "linear.app" ]]; then
    # Record the --data argument
    while [[ \$# -gt 0 ]]; do
        if [[ "\$1" == "--data" ]]; then
            echo "\$2" > "${TEST_DIR}/last_request.json"
            break
        fi
        shift
    done
...
EOF
    ...
    [ -f "${TEST_DIR}/last_request.json" ]
    # JSON escaping is preserved by curl mock — Triage and team names appear as
    # \"Triage\", \"Dev Ops\", \"AI\" inside the GraphQL query string.
    grep -q 'Triage' "${TEST_DIR}/last_request.json"
    grep -q 'Dev Ops' "${TEST_DIR}/last_request.json"
    grep -q '"AI"\|\\"AI\\"' "${TEST_DIR}/last_request.json"
}