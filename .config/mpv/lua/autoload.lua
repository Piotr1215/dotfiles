...
function alphanumsort(filenames)
    ...
    -- Capture order: cameras number photos and videos in one sequence
    -- (GP011790.JPG, GX011791.MP4, GP011792.JPG), so sort by the first number
    -- in the name before the alphanumeric key. Names without a number keep
    -- the plain alphanumeric order.
    local function firstnum(f)
        local n = f:match("%d+")
        return n and tonumber(n) or -1
    end
    local tuples = {}
    for i, f in ipairs(filenames) do
        tuples[i] = {f:lower():gsub("0*(%d+)%.?(%d*)", padnum), f, firstnum(f)}
    end
