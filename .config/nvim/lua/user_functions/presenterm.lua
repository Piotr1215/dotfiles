...
-- Telescope partial picker for including markdown partials
function M.partial_picker()
  local ok = pcall(require, "telescope")
  if not ok then
    vim.notify("Telescope not found", vim.log.levels.ERROR)
    return
  ...
  local partials_dir = git_root .. "/_partials"
  if vim.fn.isdirectory(partials_dir) ~= 1 then
    vim.notify("_partials directory not found", vim.log.levels.WARN)
    return
  end
  ...
  if #partial_files == 0 then
    vim.notify("No partial files found in _partials/", vim.log.levels.WARN)
    return
  end
  ...
  for _, filepath in ipairs(partial_files) do
    ...
    table.insert(entries, {
      ...
      path = filepath,
      relative_path = "../_partials/" .. filename,
  ...
  pickers
    .new({}, {
      prompt_title = "Presenterm Partials",
      finder = finders.new_table {
        results = entries,
        entry_maker = function(entry)
          return {
      ...
      previewer = previewers.new_buffer_previewer {
        ...
        define_preview = function(self, entry, status)
          vim.api.nvim_buf_set_lines(self.state.bufnr, 0, -1, false, entry.value.preview_lines)
      ...
      attach_mappings = function(prompt_bufnr, map)
        actions.select_default:replace(function()
          ...
          if selection then
            ...
            local include_line = "<!-- include: " .. selection.value.relative_path .. " -->"
            vim.api.nvim_put({ include_line }, "", true, true)
            vim.notify("Inserted: " .. include_line, vim.log.levels.INFO)
          end
        end)
        return true
      end,
...
local function setup_buffer_keymaps()
  ...
  vim.keymap.set(
    ...
    "<leader>sp",
    M.partial_picker,
    vim.tbl_extend("force", opts, { desc = "Include partial (telescope)" })
  )
...
function M.show_keybindings()
  ...
  local bindings = {
    ...
    "",
    "## Partials",
    "<leader>sp  - Include partial from _partials/",
    "",
    "## Code Blocks",
