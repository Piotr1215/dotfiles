...
function M.telescope_pods()
  ...
  for _, pod in ipairs(pods_data.items) do
    ...
    table.insert(pods, {
      ...
      display = string.format("%-50s %-10s %s", pod.metadata.name, status, ready and total and string.format("%d/%d", ready, total) or "0/0"),
    })
  end
...
function M.telescope_services()
  ...
  for _, svc in ipairs(services_data.items) do
    ...
    table.insert(services, {
      ...
      display = string.format("%-30s %-15s %-15s %s", svc.metadata.name, svc.spec.type, svc.spec.clusterIP or "None", table.concat(ports, ",")),
    })
  end
...
function M.telescope_contexts()
  ...
  for _, ctx in ipairs(contexts_data.contexts or {}) do
    ...
    table.insert(contexts, {
      ...
      current = is_current,
      display = string.format("%s%-30s %-20s %-15s", is_current and "* " or "  ", ctx.name, ctx.context.cluster, ctx.context.namespace or "default"),
    })
  end
...
function M.kubectl_delete_resource(resource_type, resource_name, namespace)
  vim.ui.select({ "Yes", "No" }, { prompt = string.format("Delete %s %s?", resource_type, resource_name) }, function(choice)
    if choice == "Yes" then
...
function M.kubectl_exec_shell(pod_name, namespace)
  local containers = run_kubectl_command { "get", "pod", pod_name, "-n", namespace, "-o", "jsonpath={.spec.containers[*].name}" }
  ...
  if #container_list > 1 then
    vim.ui.select(container_list, { prompt = "Select container:" }, function(container)
      if container then
        local cmd = string.format("kubectl exec -it %s -c %s -n %s -- /bin/bash || kubectl exec -it %s -c %s -n %s -- /bin/sh", pod_name, container, namespace, pod_name, container, namespace)
        vim.cmd("split | terminal " .. cmd)
      end
    end)
  else
    local cmd = string.format("kubectl exec -it %s -n %s -- /bin/bash || kubectl exec -it %s -n %s -- /bin/sh", pod_name, namespace, pod_name, namespace)
    vim.cmd("split | terminal " .. cmd)
  end
end
  ...
  vim.ui.input({ prompt = "Local port:" }, function(local_port)
    if local_port then
      vim.ui.input({ prompt = "Pod port:" }, function(pod_port)
        if pod_port then
          local cmd = string.format("kubectl port-forward %s %s:%s -n %s", pod_name, local_port, pod_port, namespace)
          vim.cmd("split | terminal " .. cmd)
        end
      end)
    end
...
vim.api.nvim_set_keymap("n", "<leader>kp", ":lua require('user_functions.kubernetes').telescope_pods()<CR>", { noremap = true, silent = true, desc = "Kubernetes Pods" })
vim.api.nvim_set_keymap("n", "<leader>kD", ":lua require('user_functions.kubernetes').telescope_deployments()<CR>", { noremap = true, silent = true, desc = "Kubernetes Deployments" })
vim.api.nvim_set_keymap("n", "<leader>ks", ":lua require('user_functions.kubernetes').telescope_services()<CR>", { noremap = true, silent = true, desc = "Kubernetes Services" })
vim.api.nvim_set_keymap("n", "<leader>kn", ":lua require('user_functions.kubernetes').telescope_namespaces()<CR>", { noremap = true, silent = true, desc = "Kubernetes Namespaces" })
vim.api.nvim_set_keymap("n", "<leader>kc", ":lua require('user_functions.kubernetes').telescope_contexts()<CR>", { noremap = true, silent = true, desc = "Kubernetes Contexts" })
vim.api.nvim_set_keymap("n", "<leader>kA", ":lua require('user_functions.kubernetes').kubectl_get_all()<CR>", { noremap = true, silent = true, desc = "Kubernetes Get All" })
