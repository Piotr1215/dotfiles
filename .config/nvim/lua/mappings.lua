...
-- SAVE & CLOSE --
utils.lnmap("wa", ":wa<cr>", { desc = "save all" })
...
utils.lnmap("nn", ":.norm ", { silent = false, desc = "normal mode command on current line" })
utils.lnmap("NN", ":%norm ", { silent = false, desc = "normal mode command on all lines" })
...
vim.keymap.set("n", "<leader>ex", function()
  local line = vim.api.nvim_get_current_line()
  vim.fn.system("bash -e", line)
end, { silent = true, desc = "execute current line silently" })
